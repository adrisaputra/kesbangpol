            ...
            <div class="box-header with-border">
              <div class="box-tools pull-left" style="float: left;">
                <div style="padding-top:10px">
                  @if(Request::segment(1)!="status_skt_ormas_w")
                    <a href="{{ url('/buat_pengajuan_skt_ormas_w') }}" class="btn btn-success btn-flat" title="Buat Pengajuan" onclick="return confirm('Anda Yakin ?');">Buat Pengajuan</a>
                    <a href="{{ url('/pengajuan_skt_ormas_w') }}" class="btn btn-warning btn-flat" title="Refresh halaman">Refresh</a>   
                  @else
                    <a href="{{ url('/status_skt_ormas_w') }}" class="btn btn-warning btn-flat" title="Refresh halaman">Refresh</a>   
                  @endif 
                </div>
              </div>
              <div class="box-tools pull-right" style="float: right;margin-top: 10px;">
                <div class="form-inline">
                @if(Request::segment(1)=="pengajuan_skt_ormas_w")
                  <form action="{{ url('/pengajuan_skt_ormas_w/search') }}" method="GET">
                @else
                  <form action="{{ url('/status_skt_ormas_w/search') }}" method="GET">
                @endif
            ...
            <div class="table-responsive table-download box-body">
                ...
                @foreach($data as $v)
                    <tr>
                      ...
                      <td>
                        @if($v->status==0)
                          <a href="{{ asset('/pengajuan_skt_ormas_w/edit/'.$v->id) }}" class="btn btn-sm btn-danger btn-flat">Upload Dokumen<i class="icofont-download"></i></a>
                        @elseif($v->status==1 || $v->status==2 ||$v->status==3 )
                          <a href="{{ asset('/pengajuan_skt_ormas_w/detail/'.$v->id) }}" class="btn btn-sm btn-primary btn-flat">Lihat Dokumen<i class="icofont-download"></i></a>
                        @elseif($v->status==5 )
                          <a href="{{ asset('/pengajuan_skt_ormas_w/perbaikan/'.$v->id) }}" class="btn btn-sm btn-primary btn-flat">Perbaiki Dokumen<i class="icofont-download"></i></a>
                        @elseif($v->status==4)
