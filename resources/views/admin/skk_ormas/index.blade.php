...
<div class="content-wrapper">
	...
	<div class="box">   
		<div class="box-header with-border">
			...
			<div class="table-responsive box-body">
				...
				<table class="table table-bordered">
					...
					@foreach($skk_ormas as $v)
					<tr>
						<td>{{ ($skk_ormas ->currentpage()-1) * $skk_ormas ->perpage() + $loop->index + 1 }}</td>
						<td>{{ $v->kode }}</td>
						<td>{{ $v->nik }}</td>
						<td>{{ $v->name }}</td>
						...
						<td>
							...
							@if(Request::segment(1)=="skk_ormas_selesai")
		...
		<div class="box-footer">
			<!-- PAGINATION -->
			<div class="float-right">{{ $skk_ormas->appends(Request::only('search'))->links() }}</div>
		</div>
	</div>
	</section>
</div>
